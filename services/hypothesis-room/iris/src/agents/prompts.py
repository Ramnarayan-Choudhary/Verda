...
# Paper-grounded ideation: user uploads a paper + provides a directive
IDEATION_GENERATE_FROM_PAPER_PROMPT = """Act as an experienced researcher specializing in the area of the provided paper. Your goal is to generate a high-impact forward hypothesis or research idea that builds upon, extends, or is inspired by the source paper.
...
Based on the source paper and the user's directive, generate **one** novel and significant research idea. The idea should:
- Build upon the findings, methods, or gaps identified in the source paper
- Address the user's specific directive (e.g., forward hypothesis, extension to new domain, improvement of method)
- Be substantially novel — not a trivial extension of the paper's existing contributions

Present the idea as a structured JSON object with the following fields:

{{
  "title": "A concise, impactful title capturing the core research question or contribution.",
  "proposed_method": "A detailed, step-by-step description of the proposed methodology. Explicitly reference how it relates to or extends the source paper. Include key components, algorithmic procedures, architectural details (if applicable), and the underlying intuition or theoretical justification.",
  "experiment_plan": "A concrete plan for empirical validation. Specify: \\n- **Datasets:** Standard benchmark datasets suitable for the task.\\n- **Baselines:** Key state-of-the-art methods for comparison (including the source paper's method).\\n- **Metrics:** Primary evaluation metrics relevant to the research goal.\\n- **Ablation Studies:** Specific experiments to isolate and validate the contribution of key components.",
}}

### Instructions:
1. **Ground in the Source Paper:** Your idea must clearly connect to the paper's contributions, limitations, or open questions.
2. **Honor the User's Directive:** If the user asks for forward hypotheses, generate hypotheses. If they ask to extend to a domain, focus on that domain.
3. **Ensure Novelty:** The idea should go beyond what the paper already does — propose something the authors did not.
4. **Maintain Feasibility:** The proposed method and experiments must be realistically achievable.
5. **Adhere to Format:**
    * Generate *only* the JSON object as output. No introductory or concluding text.
    * Use Markdown formatting within the JSON string values. Do *not* embed nested JSON within string values.
    * Do not use json within the fields only markdown formatting.
