...
from .prompts import (
    ...
    IDEATION_GENERATE_FROM_PAPER_PROMPT,
...
DEBUG_VERBOSE = os.environ.get("IRIS_DEBUG_VERBOSE", "false").lower() == "true"
...
class IdeationAgent(BaseAgent):
    ...
    def __init__(self, config_path: str):
        ...
        # self.model = self.config["ideation_agent"].get("model", "gemini/gemini-2.5-flash-lite")
        self.model = self.config["ideation_agent"].get("model", "gemini/gemini-2.5-flash-lite")
    ...
    @retry.retry(tries=4, delay=2, backoff=2)
    ...
    def execute_action(self, action: str, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Guard: LLM can return None (e.g. safety filter, empty response)
            if not content:
                logger.warning(f"LLM returned empty content for action {action}")
                content = ""
            ...
            if action == "generate_query":
                ...
                if query:
                    ...
                    return {"content": content or "hypothesis generation query"}
            ...
            if parsed_data:
                ...
                # If structured formatting produced empty content, fall back to raw
            ...
            return {"title": "Untitled Research Idea", "content": content or "No content generated.", "raw_llm_output": content}
    ...
    def _get_action_prompt(self, action: str, state: Dict[str, Any]) -> str:
        ...
        elif action == "generate_from_paper":
            # Paper-grounded ideation: user uploaded a paper + typed a directive
            paper_context = state.get("paper_context", "")
            user_directive = research_goal or current_idea
            memory_context = self.get_memory_context()
            prompt = IDEATION_GENERATE_FROM_PAPER_PROMPT.format(
