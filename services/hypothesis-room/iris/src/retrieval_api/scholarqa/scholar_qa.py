...
class ScholarQA:
    def __init__(
        ...
        if deploy_mode:
            ...
            # Gemini is the only strictly required key for the default local setup.
            required_keys = {
    ...
    def run_qa_pipeline(self, req: ToolRequest, inline_tags=False) -> TaskResult:
        ...
        if not retrieved_candidates:
            logger.warning("No retrieval candidates found for query: %s", query)
            return TaskResult(sections=[], cost=event_trace.total_cost)
        ...
        if reranked_df.empty:
            logger.warning("No relevant papers found after reranking for query: %s", query)
            return TaskResult(sections=[], cost=event_trace.total_cost)
        ...
        if not per_paper_summaries.result:
            logger.warning("No relevant quotes extracted for query: %s", query)
            return TaskResult(sections=[], cost=event_trace.total_cost)
        ...
        if not any([len(d) for d in plan_json.values()]):
            logger.warning("Planning step failed to cluster relevant documents for query: %s", query)
            return TaskResult(sections=[], cost=event_trace.total_cost)
