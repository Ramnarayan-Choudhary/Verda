...
CACHE_ENABLED = os.environ.get("ENABLE_LITELLM_CACHE", "false").lower() == "true"
...
def setup_llm_cache(cache_type: str = "s3", **cache_args):
    if not CACHE_ENABLED:
        logger.warning(
            "LiteLLM cache is disabled. Set ENABLE_LITELLM_CACHE=true to re-enable it after upgrading LiteLLM."
        )
