...
function sendMessage() {
    ...
    if (isFirstMessage) { // Check welcome message visibility instead of current_root
        ...
        // Mark that first message has been sent (used by paper badge logic)
        input[0].dataset.firstMessageSent = "true";
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Show input mode indicator if paper-grounded
            if (data.input_mode === "paper_query" && isFirstMessage) {
                var modeDiv = $('<div></div>')
                    .attr('data-sender', 'system')
                    .css({'color': '#4338ca', 'font-style': 'italic', 'font-size': '0.85em'})
                    .text('Mode: Paper-grounded ideation (idea is based on your uploaded paper)')
                    .hide();
                chatArea.append(modeDiv);
                modeDiv.slideDown();
            }
