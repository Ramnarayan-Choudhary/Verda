...
VERBOSE_DEBUG = os.environ.get("IRIS_DEBUG_VERBOSE", "false").lower() == "true"


def is_placeholder_api_key(value):
    """Detect obvious placeholder values so startup fails more clearly."""
    if not value:
        return False
...
def load_local_env_file():
    ...
    for raw_line in env_path.read_text().splitlines():
        ...
        if key and (key not in os.environ or not existing.strip() or is_placeholder_api_key(existing)):
...
# Don't use MAX_CONTENT_LENGTH — Flask abruptly closes the connection
# which causes "Failed to fetch" in the browser. We check size manually in the upload route.
...
def get_api_key(key_name, config_dict):
    ...
    if env_key:
        if is_placeholder_api_key(env_key):
            logger.warning("%s is set to a placeholder value. Replace it with a real API key.", env_var_name)
            return None
        return env_key
    ...
    if "keys" in config_dict and key_name in config_dict["keys"]:
        config_key = config_dict["keys"][key_name]
        if is_placeholder_api_key(config_key):
            logger.warning("%s in config is still a placeholder value. Replace it with a real API key.", key_name)
            return None
        return config_key
...
if HuggingFaceReranker is not None:
    ...
    logger.warning(
        "Optional reranker dependencies are not installed. Falling back to retrieval without reranking."
    )
    paper_finder = PaperFinder(retriever, context_threshold=0.1)

scholar_qa = ScholarQA(paper_finder=paper_finder, llm_model="gemini/gemini-2.5-flash-lite")
...
def test_upload_page():
    """Simple standalone upload test page for debugging."""
    return '''<!DOCTYPE html>
<html><head><title>Upload Test</title></head>
<body style="font-family: sans-serif; max-width: 600px; margin: 40px auto; padding: 20px;">
<h2>IRIS Upload Test</h2>
<p>Select a PDF to test the upload endpoint directly:</p>
<input type="file" id="file-input" accept=".pdf,.txt,.doc,.docx">
<div id="status" style="margin-top: 20px; padding: 12px; border-radius: 6px; display: none;"></div>
<pre id="log" style="margin-top: 12px; background: #f5f5f5; padding: 12px; white-space: pre-wrap; display: none;"></pre>
<script>
document.getElementById('file-input').addEventListener('change', function(e) {
    ...
    xhr.onload = function() {
        log.textContent += 'Status: ' + xhr.status + '\\nResponse: ' + xhr.responseText;
        if (xhr.status === 201) {
            status.style.background = '#d4edda';
            status.textContent = 'Upload successful!';
            try {
                var data = JSON.parse(xhr.responseText);
                status.textContent += ' File: ' + data.filename + ' | Abstract: ' + (data.abstract || 'none').substring(0, 100);
            } catch(e) {}
        } else {
    ...
    xhr.onerror = function() {
        status.style.background = '#f8d7da';
        status.textContent = 'Network error — request failed to reach server';
        log.textContent += 'XHR onerror fired. readyState=' + xhr.readyState;
    };
...
@app.route("/api/input_mode", methods=["GET"])
def get_input_mode():
    """Check if papers are uploaded and return the current input mode."""
    papers = []
    for chunk in knowledge_chunks:
        if chunk.get("file_type") == "attachment":
            papers.append({
                "filename": chunk.get("text", "").replace("Uploaded file: ", ""),
                "has_abstract": bool(chunk.get("abstract") and chunk["abstract"] != "No abstract found."),
            })
    has_paper = len(papers) > 0
    return jsonify({
        "mode": "paper_query" if has_paper else "query_only",
        "papers": papers,
        "has_paper": has_paper,
    })
...
def chat():
    ...
    if request.method == "GET":
        ...
        # force_init: True forces re-initialization with current knowledge_chunks
        # even when current_root is already set (e.g. when a new paper is loaded mid-session)
        force_init = bool(data.get("force_init", False))
        if force_init:
            current_root = None
            current_node = None
            current_state = None
        ...
        try:
            ...
            if current_root is None:
                # Detect input mode: paper+query vs query-only
                # Check if any papers have been uploaded to knowledge_chunks
                paper_text = ""
                abstract_text = ""
                paper_filename = ""
                # Accept both PDF uploads ("attachment") and arXiv metadata chunks ("knowledge")
                for chunk in knowledge_chunks:
                    if chunk.get("file_type") in ("attachment", "knowledge"):
                        paper_text = chunk.get("full_text", "")
                        abstract_text = chunk.get("abstract", "")
                        # Derive a display name for the paper
                        source = chunk.get("source", "")
                        paper_filename = (
                            chunk.get("text", "").replace("Uploaded file: ", "")
                            if chunk.get("file_type") == "attachment"
                            else source or "Loaded paper"
                        )
                        break

                has_paper = bool(paper_text and len(paper_text.strip()) > 100)

                if has_paper:
                    # --- MODE A: Paper + Query ---
                    # Truncate paper to ~8000 chars to fit context window
                    ...
                    # Prepend abstract for clarity if available
                    if abstract_text and abstract_text != "No abstract found.":
                        paper_context = f"**Abstract:** {abstract_text}\n\n**Full Paper Content:**\n{truncated_paper}"
                    else:
                        paper_context = truncated_paper

                    chat_messages.append(
                        {"role": "system", "content": f"Paper detected: {paper_filename}. Generating idea grounded in paper..."}
                    )

                    # Create root state with paper context
                    root_state = MCTSState(
                        research_goal=user_message,
                        current_idea=user_message,
                        retrieved_knowledge=[abstract_text],
                        feedback={},
                        reward=0.0,
                        depth=0,
                    ...
                    # --- MODE B: Query only (original behavior) ---
                    chat_messages.append(
                ...
                llm_response = response.get("content") or ""
                # If the LLM returned nothing meaningful, surface the raw output
                if not llm_response.strip():
                    llm_response = response.get("raw_llm_output") or ""
                if not llm_response.strip():
                    llm_response = f"[IRIS could not generate a hypothesis. Model returned empty response. Please try again or check the backend logs.]"

                mode_label = "paper-grounded" if has_paper else "query-based"
                chat_messages.append({"role": "system", "content": f"Initial idea generated ({mode_label})."})
                ...
                first_idea_state = MCTSState(
                    ...
                    retrieved_knowledge=[abstract_text] if abstract_text else [],
                    feedback={},
                    reward=0.0,
                    depth=1,
                    paper_context=paper_text[:8000] if has_paper else None,
                )

                # Only run review if we have a real idea (not an error placeholder)
                if not llm_response.startswith("[IRIS could not"):
                    review_data = review_agent.unified_review(llm_response)
                    print(f"Review score: {review_data.get('average_score', 0)}")
                    if review_data:
            ...
            # Subsequent messages: Treat as direct feedback to improve the current idea
                ...
                new_state = MCTSState(
                    ...
                    paper_context=getattr(current_node.state, "paper_context", None),
                )
            ...
            has_paper_ctx = bool(getattr(current_node.state, "paper_context", None))
            return jsonify(
                {
                    ...
                    "review_feedback": getattr(current_node.state, "review_feedback", {}),
                    "average_score": getattr(current_node.state, "average_score", 0.0),
                    "input_mode": "paper_query" if has_paper_ctx else "query_only",
                }
            )
...
def reset_session():
    """Clear all IRIS session state so a fresh paper/topic can be started."""
    global main_idea, current_root, current_node, current_state, exploration_in_progress, chat_messages, knowledge_chunks
    current_root = None
    current_node = None
    current_state = None
    exploration_in_progress = False
...
def upload_file():
    try:
        ...
        if not allowed_file(file.filename):
            return jsonify({"error": f"File type not allowed. Allowed: txt, pdf, doc, docx"}), 400

        filename = secure_filename(file.filename)
        if not filename:
            return jsonify({"error": "Invalid filename"}), 400

        # Check file size manually (read into memory, check, then save)
        file_bytes = file.read()
        max_size = 16 * 1024 * 1024  # 16MB
        if len(file_bytes) > max_size:
            return jsonify({"error": f"File too large ({len(file_bytes) // (1024*1024)}MB). Maximum is 16MB."}), 400
        ...
        if file_path.lower().endswith('.pdf'):
            try:
                ...
                logger.info(f"PDF extracted: {len(file_content)} chars, abstract: {abstract[:80]}...")
            except Exception as pdf_err:
                logger.error(f"Error extracting PDF content: {pdf_err}")
                file_content = "PDF text extraction failed"
        ...
        # Return lightweight response (don't send full_text back to browser)
        return jsonify({
