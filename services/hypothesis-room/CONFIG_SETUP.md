...
This module runs locally today with mocked keys, but production deploys should replace the placeholder values below. Keep this file alongside `.env.example` so anyone can wire up real providers quickly.

| Capability | Placeholder | Replace With When Ready |
|------------|-------------|-------------------------|
| **Primary LLM (Proposer/Critic/Evolver/Judge)** | `K2THINK_API_KEY=your-k2think-api-key` | Claude 3.5 Sonnet or GPT-4o keys. Update `.env` and optionally `LLM_PROVIDERS.md` if you swap SDKs. |
...
| **Vector Store** | Supabase fields left blank in `.env.example` | Set `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`. The vector store automatically uses pgvector (via `vecs`) once these are present. |
...
1. **Create `.env`** by copying `.env.example` and replacing the placeholders above.
...
Always restart the FastAPI server after updating `.env` so the pydantic settings reload.
