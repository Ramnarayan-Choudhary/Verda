...
import { buildFallbackPaperAnalysis } from './fallback';
...
export async function runInitialAnalysis(
    ...
    if (arxivId) state.arxiv_id = arxivId;
    ...
    try {
        ...
        logger.warn('Parser Agent failed, using fallback parser output', {
            documentId,
            error: error instanceof Error ? error.message : String(error),
        });

        try {
            state.paper_analysis = buildFallbackPaperAnalysis(paperContext);
        } catch (fallbackError) {
            state.phase = 'error';
            state.errors.push({
                phase: 'parsing' as StrategistPhase,
                message: `Fallback parser failed: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`,
                timestamp: new Date().toISOString(),
            });
            logger.error(
...
/**
 * STAGE 2: Hypothesis Generation — handled by Python service (services/hypothesis-room)
 * See: apps/web/src/app/api/strategist/hypothesize/route.ts
 * The route calls the Python service directly; no TS hypothesis logic needed here.
 */
